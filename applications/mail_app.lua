...
local mails = main:addScrollableFrame():setSize(width - 2, height - 4):setPosition(2, 3):setForeground(config.user.color.text)
...
local toinput = mailwindow:addInput():setPosition(10, 5):setSize(width - 10, 1):setForeground(config.user.color.text):setInputType("text")
local fromlabel = mailwindow:addLabel():setPosition(2, 3):setText("FROM"):setForeground(config.user.color.text)
local frominput = mailwindow:addInput():setPosition(10, 3):setSize(width - 10, 1):setForeground(config.user.color.text):setInputType("text")
local subjectlabel = mailwindow:addLabel():setPosition(2, 7):setText("SUBJECT"):setForeground(config.user.color.text)
local subjectinput = mailwindow:addInput():setPosition(10, 7):setSize(width - 10, 1):setForeground(config.user.color.text):setInputType("text")
local contentlabel = mailwindow:addLabel():setPosition(2, 9):setText("CONTENT"):setForeground(config.user.color.text)
local contentinput = mailwindow:addTextfield():setPosition(2, 10):setSize(width - 2, (height - 10) - 2):setForeground(config.user.color.text)
local channellabel = mailwindow:addLabel():setPosition(2, height - 1):setText("ADDRESS"):setForeground(config.user.color.text)
local channelinput = mailwindow:addInput():setPosition(10, height - 1):setSize(width - 10, 1):setForeground(config.user.color.text):setInputType("text")
--labels
...
mailwindow:addLabel():setPosition(getCenter("Create Mail"), 1):setText("Create Mail"):setForeground(config.user.color.text)
--func
local posY = 2
...
local function sendMail()
    ...
    if toinput:getValue() and frominput:getValue() and subjectinput:getValue() and channelinput:getValue() then
        mail.send(frominput:getValue(), toinput:getValue(), subjectinput:getValue(), textutils.serialiseJSON(content), channelinput:getValue())
    end
end
--buttons
main:addButton():setPosition(width - 6, 1):setSize(6, 1):setText(" EXIT "):setBackground(colors.red):onClick(function()os.queueEvent("terminate")end):setForeground(config.user.color.text)
main:addButton():setPosition(2, 1):setSize(14, 1):setText(" Create Mails "):setBackground(colors.blue):onClick(function()main:hide()mailwindow:show()end):setForeground(config.user.color.text)
mailwindow:addButton():setPosition(width - 6, 1):setSize(6, 1):setText(" EXIT "):setBackground(colors.red):onClick(function()mailwindow:hide()main:show()end):setForeground(config.user.color.text)
mailwindow:addButton():setPosition(2, 1):setSize(12, 1):setText(" Send Mails "):setBackground(colors.blue):onClick(sendMail):setForeground(config.user.color.text)
